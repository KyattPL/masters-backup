...
\begin{itemize}
    \item Wykorzystanie LLM do generowania questów
    \item Generowanie map i instancji assetów na podstawie pliku JSON
    \item LARP jako teoretyczny szkielet do autonomicznych agentów
    \item AI Town jako przykład autonomicznych agentów istniejących w świecie gry
\end{itemize}

Wraz z ostatnimi postępami w dziedzinie przetwarzania języka naturalnego (NLP), duże modele językowe,
takie jak BERT i GPT-2, zyskały na popularności w generowaniu narracji. W badaniu Al-Nassar i in. (2023)
zbadano potencjał tych modeli do proceduralnego generowania zadań (questów) w grze QuestVille. Podejście
to polegało na połączeniu dwóch modeli NLP: BERT i GPT-2 w sekwencji. Najpierw zdefiniowano krótkie
zdania będące podstawowymi zadaniami, z jednym słowem zamaskowanym. Zdania te przesyłano do modelu
BERT, który predykował najbardziej prawdopodobne słowa pasujące do kontekstu. Następnie losowo wybierano
jedno ze zwróconych słów i umieszczano je w zdaniu, tworząc prompt dla modelu GPT-2. Model ten generował
...
zadań z narracją, bardziej wciągających niż podstawowe polecenia. Prefixowanie promptów relacjami
między postaciami niesterowalymi w grze kierowało generowaną treść w odpowiedni kontekst. Jednak
...
Wraz z rozwojem zaawansowanych modeli językowych, takich jak GPT-3 od OpenAI, pojawiają się nowe
możliwości ich zastosowania w generowaniu interaktywnej narracji w grach wideo. Jak wskazano w
przeanalizowanej pracy (chatgpt narrative system.pdf), modele te mogą odgrywać kluczową rolę w
dynamicznym tworzeniu treści gry na podstawie wejściowych danych tekstowych i kontekstu rozgrywki.
Opisany prototyp gry wykorzystuje model ChatGPT do generowania początkowej sceny narracyjnej oraz
...
właściwej równowagi pomiędzy swobodą generatywną modelu a wymogami spójnego, graczytelnego doświadczenia.
...
kluczowe elementy stanu gry są przekazywane do generowania narracji. Podsumowując, zaprezentowany
...
obiecujące możliwości, jak i obecne ograniczenia takiego podejścia.

Duże modele językowe (LLM) zyskały w ostatnich latach na znaczeniu dzięki postępom w metodach uczenia
maszynowego oraz ogromnym zasobom danych tekstowych używanych do ich trenowania. W pracy "LARP:
LANGUAGE-AGENT ROLE PLAY FOR OPEN-WORLD GAMES" autorzy proponują wykorzystanie LLM jako podstawę dla
generatywnych agentów w środowiskach otwartych światów gier fabularnych. Kluczowym elementem architektury
...
mechanizmu dopasowywania różnorodnych osobowości i perspektyw dla agentów za pomocą klastra
drobniejszych, wyspecjalizowanych modeli LLM. Pozwala to na generowanie narracji dostosowanych do
zróżnicowanych tożsamości, stylów językowych i postaw bohaterów niezależnych. Podejście LARP łączy
...
fabularnych. Stanowi ono obiecujący kierunek badań na styku sztucznej inteligencji, projektowania gier
oraz narracji interaktywnych.

TODO BO CLAUDE LIMIT